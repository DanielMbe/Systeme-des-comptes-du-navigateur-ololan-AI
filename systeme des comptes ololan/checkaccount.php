<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET") {
        session_destroy();
        session_start();
        $_SESSION['user_id_disconnect'] = mb_strlen($_GET['uname']);
        $_SESSION['email'] = $_GET['ulogin'];
        $_SESSION['password'] = $_GET['upassword'];
        $_SESSION['username'] = $_GET['uname'];
        $_SESSION['loginID'] = $_GET['loginID'];
        $requestType = $_GET['reqType'];
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT * FROM ololanaccountusers WHERE email = :email AND epassword = :epassword");
            $stmt->bindParam(':email', $_SESSION['email']);
            $stmt->bindParam(':epassword', $_SESSION['password']);
            $stmt->execute();

            // check if the user exists
            if ($stmt->rowCount() === 1) {
                //process disconnection
                if (strcmp($requestType, "logout") == 0) {
                    header("Location: https://accounts.ololan.com/logout");
                } else if (strcmp($requestType, "delete") == 0) {
                    header("Location: https://accounts.ololan.com/accounts");
                }else {
